...
def build_shield():
    """"Verified parental consent" — a wooden shield with a brass check.

    The one point that is a legal fact rather than a product object, and for
    a year it wore a stroked line icon for that reason. Patrick, 2026-09-01:
    beside a photographed key and chest a generic security icon reads as a
    different vocabulary, so the shield became an object in the same
    material language — generated by the app repo's gen-desk-object.py
    (`shield`), keyed to transparent there, and shared with the App Store
    screenshots so the two never drift.
    """
    return bed(fit_h(load("samples/desk_objects/safety_shield_transparent.png"), H))


if __name__ == "__main__":
    for name, fn in (("safety-closed-box.png", build_closed_box),
                     ("safety-drawers.png",    build_drawers),
                     ("safety-shield.png",     build_shield)):
        im = fn()
